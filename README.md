> v11.25：实时教师后台已改为轻量状态轮询 + 当前学生增量更新，并新增实时行为轨迹；未发送草稿正文不保存。

# Design Thinking + GenAI Help-Seeking Research Platform v11.25
...
## 旧数据保留
重新部署 v11.25 不会因为代码更新清空 W1。存储根路径仍为：
...
## v11.25 教师实时监看与数据安全
- 后台新增“实时AI对话”：约3秒刷新，左侧学生列表显示状态/轮数/最近任务步骤，点击编号右侧直接显示当前课次完整对话。
- 学生消息在等待AI回复前先落盘，因此教师可以在AI处理期间实时看到学生刚发出的内容；状态显示“AI处理中 / 已回复 / 异常”。
- 新增“历史课次概览”，切换当前课次后仍可看到W1/W2等历史记录数量。
- 新增只读“存储诊断”，显示当前存储根前缀和每课次record/message/event/revision对象数量。
- 后台“重置”产生的归档可在当前目标课次为空时人工恢复。
- UI关闭“整批更换学生（清空旧数据）”；服务端默认也拒绝该破坏性操作。普通名单更新永远保留已有研究数据。
